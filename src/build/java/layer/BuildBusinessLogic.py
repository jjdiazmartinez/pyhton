...
class BuildBusinessLogic:
    def build(self, structureJson):
        templ = template.SharedTemplate()
        self.template = templ.getTemplateBusinesslogic()
    ...
    def doHead(self):
        ...
        self.template = self.template.replace("<package>", base["groupId"] + ".businesslogic." + self.structure["name"].lower())
